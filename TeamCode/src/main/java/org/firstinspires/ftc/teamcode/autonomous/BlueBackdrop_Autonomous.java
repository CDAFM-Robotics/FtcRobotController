...
@Autonomous(group = "Competition", name = "BlueBackdrop_Autonomous")

public class BlueBackdrop_Autonomous extends LinearOpMode {
  ...
  public void RR_BZ1_Pixel()
  {
    ...
    // Move to Drop on point
    .splineToLinearHeading(new Pose2d(-36, 31.80, Math.toRadians(0.00)), Math.toRadians(0.00)) // x:-34.5 ; x:-32.5
  ...
  public void RR_BZ3_Pixel()
  {
    ...
    .lineToConstantHeading(new Vector2d(45, 0)) // x:48
    .build();
  ...
  public void RR_BZ2_Backdrop()
  {
    ...
    .lineToLinearHeading(new Pose2d(64, 65, Math.toRadians(180.00))) // y:63
    .build();
  ...
  public void RR_BZ3_Backdrop()
  {
    ...
    .lineToLinearHeading(new Pose2d(64, 66, Math.toRadians(180.00))) // y:65
    .build();
...
// OK 17Nov23
  ...
  public void RR_RZ2_Backdrop()
  {
    ...
    .lineToConstantHeading(new Vector2d(12.61, -35.5))
    .addDisplacementMarker(() -> {
    ...
    .lineToLinearHeading(new Pose2d(64, -65, Math.toRadians(-180.00)))
    .build();
  ...
  public void RR_RZ1_Backdrop()
  {
    ...
    //.lineToLinearHeading(new Pose2d(12.97, -32, Math.toRadians(-180.00))) // 34
    .splineToLinearHeading(new Pose2d(13.5, -32, Math.toRadians(-180.00)), Math.toRadians(-180.00)) // TWEAK 17Nov23 (Avoid truss bump)
    .addDisplacementMarker(() -> {
      // Deploy Purple on strike 1
      // TODO: REPLACE SLEEP with something else
      bottomArmServo.setPosition(BotConstants.BOTTOM_ARM_SERVO_OPEN);
  ...
  public Vector2d acquireTagLocation(Vector2d Current, int myTeam) {
    boolean found=false;
    int tries=0;
    double edge_offset = BotConstants.CENTER_TAG_OFFSET; // 0.0
    Vector2d Future;
    ...
    while (tries<5) {
      ...
      for (AprilTagDetection detection : currentDetections) {
        if (detection.metadata != null) {
            ...
            if ((zone == 1) || (zone == 4))
            {
              edge_offset = BotConstants.LEFT_EDGE_OFFSET; // 0.75
            }
            else if ((zone == 3) || (zone == 6))
            {
              edge_offset = BotConstants.RIGHT_EDGE_OFFSET; // -0.75
            }
            Future = new Vector2d(Current.getX() +  Math.cos(BotConstants.CAM_TILT_ANGLE_RAD)*detection.ftcPose.y + BotConstants.APRIL_POSE_YOFFSET,
                                  Current.getY() - (detection.ftcPose.x + (targetId - detection.id)*BotConstants.TAG_TO_TAG_DIST) + BotConstants.APRIL_POSE_XOFFSET + edge_offset);
            found = true;
            tagIdFound = targetId;
            tagFound = found;
            if (detection.id == targetId) {
              telemetry.addData("Found ours: ", targetId);
              telemetry.addLine(String.format("!! CURRENT %6.1f %6.1f ", Current.getX(), Current.getY()));
              telemetry.addLine(String.format("!! FUTURE %6.1f %6.1f ", Future.getX(), Future.getY()));
              telemetry.update();
              return Future;
            }
      ...
      if (found)
      {
        telemetry.addData("calc", tagIdFound);
        telemetry.addLine(String.format("xCURRENT %6.1f %6.1f ", Current.getX(), Current.getY()));
        telemetry.addLine(String.format("xFUTURE %6.1f %6.1f ", Future.getX(), Future.getY()));
        telemetry.update();
        return Future;
      }
